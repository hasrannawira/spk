...
class Surat_distribusi extends MY_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->check_login();
        if ($this->session->userdata('id_role') != "2" or $this->session->userdata('id_satker') != "4") {
            redirect('', 'refresh');
        }
    }
    ...
    public function index()
    {
        ...
        $this->template->load('layouts/member/template', 'member/surat_distribusi', $data);
    }
    ...
    public function hapus($id_surat){
        ...
        $aktivitas = array(
                'username'          =>  $this->session->userdata('username'),
                'aktivitas'         =>  'Menghapus Surat Seksi Stat. Distribusi',
        );
        $this->m_aktivitas->input_data($aktivitas);
        redirect ('member/surat_distribusi');
    }
    ...
    public function edit($id_surat){
        ...
        $this->template->load('layouts/member/template', 'member/edit_surat_distribusi', $data);        
    }
    ...
    public function update(){
        ...
        $aktivitas = array(
                'username'           =>  $this->session->userdata('username'),
                'aktivitas'         =>  'Mengupdate Surat Seksi Stat. Distribusi',
        );
        ...
        $where = array ('id_surat' => $id_surat);
        $data['surat'] = $this->m_surat_distribusi->edit_data($where)->result();
        $this->template->load('layouts/template', 'member/edit_surat_distribusi', $data);             
        }
    }
